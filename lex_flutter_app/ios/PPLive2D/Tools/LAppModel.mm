 ...
 */
void LAppModel::load(const char path[],LModelConfig *modelConfig) {
    ...
    if(modelConfig){
        modelMatrix->centerX(modelConfig.center_x);
        modelMatrix->setY(modelConfig.y);
//        modelMatrix->setWidth(modelConfig.width);
//        modelMatrix->setHeight(modelConfig.height);
    }
...
void LAppModel::preloadMotionGroup(const char name[]) {
    ...
    for (int index = 0; index < count; index++) {
        ...
        if (LAppDefine::DEBUG_LOG) {
            NSLog(@"load motion name:%s viewTag:%d",motionFile.c_str(),modelViewTag);
        }
...
void LAppModel::update() {
	...
	live2DModel->setParamFloat(PARAM_MOUTH_FORM, mouthFormValue, 0.8f);
...
void LAppModel::setModelViewTag(int tag){
//    modelViewTag = tag;
}
/*
 * モデルを描画する。
...
int LAppModel::startMotion(const char name[], int no, int priority) {
	...
	if (motion == NULL) {
        ...
        if (LAppDefine::DEBUG_LOG) {
            NSLog(@"load motion name:%s tag:%d",motionFile.c_str(),modelViewTag);
        }
	...
	if (strcmp([modelSetting motionSoundByName:name atIndex:no], "") == 0) {
        if (LAppDefine::DEBUG_LOG) {
            NSLog(@"start motion : %s tag:%d",motionFile.c_str(),modelViewTag);
        }
        ...
        if(LAppDefine::DEBUG_LOG) {
            NSLog(@"start motion : %s  sound : %s tag:%d",motionFile.c_str(),soundName.c_str(),modelViewTag);
        }
...
void LAppModel::speakMotion(bool isSpeaking){
...
//    string motionFile = [modelSetting motionFileByName:MOTION_GROUP_SHAKE atIndex:0];
//    AMotion* motion = motions[motionFile];
//    bool autoDelete = false;
//    if(!isOff){
//        mainMotionMgr->stopAllMotions();
...
//
//        motion = Live2DMotion::loadMotion([FileManager pathForResource:path.c_str()]);
//        motion->setFadeIn((int)[modelSetting motionFadeInByName:MOTION_GROUP_SHAKE atIndex:0]);
//        motion->setFadeOut((int)[modelSetting motionFadeOutByName:MOTION_GROUP_SHAKE atIndex:0]);
//        Live2DMotion *l2dMotion = (Live2DMotion*)motion;
//        l2dMotion->setLoop(true);
//        autoDelete = true;
//    }
//    if (LAppDefine::DEBUG_LOG) {
...
//    }
//    // 終了時にメモリから削除
//    mainMotionMgr->startMotionPrio(motion, autoDelete, PRIORITY_FORCE);
}

void LAppModel::setLipSyncValue(float value){
    lipSync = true;
    lipSyncValue = ClampFloat(value, 0.0f, 1.0f);
}

void LAppModel::setMouthFormValue(float value){
    mouthFormValue = ClampFloat(value, -1.0f, 1.0f);
}
 ...
 * 从指定ID的顶点列表中计算包含它们的最大矩形，并确定点是否包含在其中
 * @param id
 * @param testX
 * @param testY
 * @return
